...
export const postReactionMenu = (post) => {
  ...
  clone
    ...
    .append(
      ...emojis.map((emoji) =>
        reactionOptionTemplate(emoji, post.id),
      ),
    );

  const reactions = clone.querySelectorAll(
    '[data-reaction]',
  );
...
export const reactionOptionTemplate = (symbol, postId) => {
  ...
  clone.querySelector('.dropdown-item').dataset.symbol =
    symbol;
  clone.querySelector('.dropdown-item').dataset.postId =
    postId;
